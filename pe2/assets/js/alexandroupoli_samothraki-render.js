/* ============================================================
   Αλεξανδρούπολη-Σαμοθράκη rendering: διαβάζει window.WB_DATA
   (inlined από alexandroupoli_samothraki-data.js).
   CORS-safe — δουλεύει και με file:// protocol.
   ============================================================ */

(function () {
  const data = window.WB_DATA;
  if (!data) { console.error('WB_DATA not loaded — include alexandroupoli_samothraki-data.js before alexandroupoli_samothraki-render.js'); return; }
  const C = window.WBCharts;
  if (!C) { console.error('WBCharts not loaded — include charts.js before alexandroupoli_samothraki-render.js'); return; }
  ...
  // 11.9 STR share — split into 2 panels
  if (data.str_market) {
  ...
  // 11.11 IPA — διπολική προσέγγιση: τα Q5_3 στάθμευση και Q5_4 ακτοπλοϊκή
  // ως Concentrate-Here στο διπολικό IPA τεταρτημόριο
  C.ipa('chart_ipa', {
    x: [0.30, 0.28, 0.26, 0.22, 0.27, 0.18, 0.20, 0.32, 0.29, 0.27, 0.19, 0.22, 0.20, 0.25, 0.24],
    y: [3.95, 4.05, 3.85, 3.65, 3.95, 2.85, 2.95, 2.45, 3.05, 3.55, 3.15, 3.40, 3.10, 3.30, 3.85],
    labels: ['Καταλύματα (επάρκεια)','Καταλύματα (ποιότητα)','Χώροι εστίασης','Τουρ. υποδομές','Διασκέδαση','Τοπικό οδικό','Σύνδεση Ελλάδα','Στάθμευση','Ακτοπλοΐα Σαμοθράκης','Δημ. Υγεία','Αστυνόμευση','Ύδρευση','Αποχέτευση','Απορρίμματα','Τηλεπ./Internet'],
  ...
  // 11.13b Quadrant Employment × SAT
  if (data.quadrants) {
  ...
  if (data.quadrants) {
    ...
    C.quadrant('chart_q_tfi_poi', {
      ...
      labels_q: { tl: 'Υποκειμενική κρίση χωρίς αντικείμενο · Ζώνη Α′', tr: 'Κορεσμένος + ενήμερος', bl: 'Λανθάνον δυναμικό', br: 'Ώριμος προσαρμοσμένος' },
    });
  }

  // 11.16c Quadrant STR × Housing
  if (data.quadrants) {
    const qd = Object.entries(data.quadrants);
  ...
  if (data.mrb_overall) {
    ...
    const dests = ['Αλεξ./πολη','Ιωάννινα','Καστοριά','Κέρκυρα','Χανιά'];
    C.heatmap('chart_doxey', {
      rows: labels,
      cols: dests,
      z: [
        [4.28, 4.49, 4.48, 4.50, 4.31],
        [4.10, 4.11, 4.15, 4.00, 4.15],
        [3.35, 2.71, 2.20, 3.39, 3.05],
        [3.30, 2.82, 2.10, 3.35, 3.12],
        [2.65, 1.90, 1.65, 2.15, 2.38],
        [2.78, 2.11, 1.55, 2.65, 2.61],
      ],
  ...
  // 11.17b Quadrant LCA polarization
  if (data.quadrants) {
    const qd = Object.entries(data.quadrants);
  ...
  // 11.18 Environmental heatmap (Q3 + Q7 cross-dest)
  if (data.mrb_cross) {
    ...
    const dest_pretty = ['Αλεξ./πολη','Ιωάννινα','Καστοριά','Κέρκυρα','Χανιά'];
    const idx = ['Q7_1','Q7_2','Q7_3','Q7_4','Q7_5','Q3_3','Q3_4','Q3_5'];
    const dest_codes = ['ΑΛΕΞΑΝΔΡΟΥΠΟΛΗ_ΣΑΜΟΘΡΑΚΗ','ΙΩΑΝΝΙΝΑ','ΚΑΣΤΟΡΙΑ','ΚΕΡΚΥΡΑ','ΧΑΝΙΑ'];
    const z = idx.map(it => {
