/* ============================================================
   Μύκονος rendering: διαβάζει window.WB_DATA (inlined από
   mykonos-data.js). CORS-safe — δουλεύει και με file:// protocol.
   ============================================================ */

(function () {
  const data = window.WB_DATA;
  if (!data) { console.error('WB_DATA not loaded — include mykonos-data.js before mykonos-render.js'); return; }
  const C = window.WBCharts;
  if (!C) { console.error('WBCharts not loaded — include charts.js before mykonos-render.js'); return; }
  ...
  } catch(e) { console.error('Mykonos chart phase crashed (sections below still render):', e); }
