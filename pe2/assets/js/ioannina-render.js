/* ============================================================
   Ιωάννινα rendering: διαβάζει window.WB_DATA (inlined από
   ioannina-data.js). CORS-safe — δουλεύει και με file:// protocol.
   ============================================================ */

(function () {
  const data = window.WB_DATA;
  if (!data) { console.error('WB_DATA not loaded — include ioannina-data.js before ioannina-render.js'); return; }
  const C = window.WBCharts;
  if (!C) { console.error('WBCharts not loaded — include charts.js before ioannina-render.js'); return; }
