/* ============================================================
   Χανιά rendering: διαβάζει window.WB_DATA (inlined από
   chania-data.js) και κατασκευάζει όλα τα interactive elements.
...
(function () {
  ...
  if (!data) {
    console.error('WB_DATA not loaded — check that chania-data.js is included before chania-render.js');
    return;
  }
  const C = window.WBCharts;
  if (!C) {
    console.error('WBCharts not loaded — check that charts.js is included before chania-render.js');
    return;
  }
